...
describe('AssemblyAiProvider', () => {
  ...
  describe('submitAndWaitByUrl', () => {
    ...
    it('should handle completed status on first poll', async () => {
      const createResponse = { status: 200, data: { id: mockTranscriptId, status: 'queued' } };
      const completedResponse = { status: 200, data: { id: mockTranscriptId, status: 'completed', text: 'Quick response', audio_duration: 1.0 } };
    ...
    it('should throw ServiceUnavailableException when create request fails', async () => {
      ...
      await expect(provider.submitAndWaitByUrl({ audioUrl: mockAudioUrl, apiKey: mockApiKey })).rejects.toThrow(ServiceUnavailableException);
    });
    ...
    it('should throw ServiceUnavailableException when status is error', async () => {
      const createResponse = { status: 200, data: { id: mockTranscriptId, status: 'queued' } };
      const errorResponse = { status: 200, data: { id: mockTranscriptId, status: 'error', error: 'Audio file format not supported' } };
      jest.spyOn(httpService, 'post').mockReturnValueOnce(of(createResponse as any));
      jest.spyOn(httpService, 'get').mockReturnValueOnce(of(errorResponse as any));
      await expect(provider.submitAndWaitByUrl({ audioUrl: mockAudioUrl, apiKey: mockApiKey })).rejects.toThrow('Audio file format not supported');
    });
    ...
    it.skip('should throw GatewayTimeoutException after maxSyncWaitMinutes', async () => {
      ...
      await expect(provider.submitAndWaitByUrl({ audioUrl: mockAudioUrl, apiKey: mockApiKey })).rejects.toThrow(GatewayTimeoutException);
    });

    it('should continue polling when response body is empty', async () => {
      const createResponse = { status: 200, data: { id: mockTranscriptId, status: 'queued' } };
      const emptyResponse = { status: 200, data: null };
      const completedResponse = { status: 200, data: { id: mockTranscriptId, status: 'completed', text: 'Recovered' } };
      jest.spyOn(httpService, 'post').mockReturnValueOnce(of(createResponse as any));
      jest
        .spyOn(httpService, 'get')
    ...
    it('should handle completed response with minimal fields', async () => {
      const createResponse = { status: 200, data: { id: mockTranscriptId, status: 'queued' } };
      const completedResponse = { status: 200, data: { id: mockTranscriptId, status: 'completed', text: 'Minimal response' } };
      jest.spyOn(httpService, 'post').mockReturnValueOnce(of(createResponse as any));
      jest.spyOn(httpService, 'get').mockReturnValueOnce(of(completedResponse as any));
      const result = await provider.submitAndWaitByUrl({ audioUrl: mockAudioUrl, apiKey: mockApiKey });
    ...
    it('should handle completed response with null text', async () => {
      const createResponse = { status: 200, data: { id: mockTranscriptId, status: 'queued' } };
      const completedResponse = { status: 200, data: { id: mockTranscriptId, status: 'completed', text: null } } as any;
      jest.spyOn(httpService, 'post').mockReturnValueOnce(of(createResponse as any));
      jest.spyOn(httpService, 'get').mockReturnValueOnce(of(completedResponse as any));
      const result = await provider.submitAndWaitByUrl({ audioUrl: mockAudioUrl, apiKey: mockApiKey });
    ...
    it('should correctly map words array excluding confidence field', async () => {
      const createResponse = { status: 200, data: { id: mockTranscriptId, status: 'queued' } };
      const completedResponse = { status: 200, data: { id: mockTranscriptId, status: 'completed', text: 'Test words', words: [ { start: 0, end: 100, text: 'Test', confidence: 0.99 }, { start: 100, end: 200, text: 'words', confidence: 0.98 } ] } };
      jest.spyOn(httpService, 'post').mockReturnValueOnce(of(createResponse as any));
      jest.spyOn(httpService, 'get').mockReturnValueOnce(of(completedResponse as any));
      const result = await provider.submitAndWaitByUrl({ audioUrl: mockAudioUrl, apiKey: mockApiKey });
      expect(result.words).toEqual([ { start: 0, end: 100, text: 'Test' }, { start: 100, end: 200, text: 'words' } ]);
      expect(result.words![0]).not.toHaveProperty('confidence');
    }, 10000);

    it('should handle empty words array', async () => {
      const createResponse = { status: 200, data: { id: mockTranscriptId, status: 'queued' } };
      const completedResponse = { status: 200, data: { id: mockTranscriptId, status: 'completed', text: '', words: [] } };
      jest.spyOn(httpService, 'post').mockReturnValueOnce(of(createResponse as any));
      jest.spyOn(httpService, 'get').mockReturnValueOnce(of(completedResponse as any));
      const result = await provider.submitAndWaitByUrl({ audioUrl: mockAudioUrl, apiKey: mockApiKey });
