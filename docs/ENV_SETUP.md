...
- `STT_MAX_FILE_SIZE_MB` — лимит размера файла, по умолчанию `100`
...
- `ALLOW_CUSTOM_API_KEY` — разрешить пользовательский API ключ в запросе, по умолчанию `false`
- `ASSEMBLYAI_API_KEY` — API ключ провайдера (используется, когда `ALLOW_CUSTOM_API_KEY=false`)
